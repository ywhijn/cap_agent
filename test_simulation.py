...
from test_episode import TestEpisode
...
def main():
    ...
    for pooling_rate in pooling_rates:
        ...
        if args.DrawDistribution:
            ...
            ax = control_center.DrawRequests(ax, requests, type='pickup', s=15, count=True, cmap='Reds', cmax=40,
                                             color='red', draw_grid=False)
            plt.subplots_adjust(left=0.15)
        ...
        for i in range(epoch_num):
            ...
            if args.DrawResult:
                img_path = os.path.join(args.OutputDir, cfg_file_name, 'tmp')
                req_num = TestEpisode(requests_tmp, vehicles_tmp, control_center, draw_veh_req=True, draw_fre=60,
                                     img_path=img_path)

            else:
                req_num = TestEpisode(requests_tmp, vehicles_tmp, control_center, draw_veh_req=False)
