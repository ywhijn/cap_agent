...
def RunEpisode(requests, vehicles, control_center, agent = None, train = False, train_step = 0, draw_veh_req = False, draw_fre = 60, img_path = 'exp'):
    ...
    for step in tqdm(range(control_center.total_steps), desc = 'Running simulation steps: '):
        ...
        # check duplicated requests in final trips
        req_set= set()
        for i in range(len(final_trips)):
            for req in final_trips[i].requests:
                if req in req_set:
                    print('duplicated requests in final trips')
                    print(req.id)
                    print(final_trips[i].requests)
                    print(final_trips)
                    print('duplicated requests in final trips')
                    exit()
                req_set.add(req)
        ...
        train_step += 1
        # img_cnt=control_center.tracker_system.draw_pooled_vehicle(control_center, img_path, step, img_cnt)
        ...
        #     ax = control_center.DrawSnapshot(ax, v_size=0.004, s=100, draw_route=False, draw_road_network=True,
        #                                      speed_lim=[5, 10], axis_lim=axis_lim)
