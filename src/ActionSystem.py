...
class ActionSystem:
    ...
    def UpdateVehicles(self, final_trips, final_paths, vehicles = None):
        ...
        for vehicle, trip, path in zip(vehicles, final_trips, final_paths):
            ...
            if path.current_position == path.next_positions[0]: #到达了目标之一，剔除目标
    ...
    def ProcessRequests(self):
        unmatched_requests = []
        for request in self.requests:#当前阶段扫描的req和之前未匹配的req
            ...
            # (2) If the request hasn't been cancelled, we can find them through vehicles
                ...
                if waiting_pickup_time > request.max_tol_pickup_time:
                    if random.random() < request.cancel_prob_pickup:
                        ...
                        if len(vehicle.next_requests) != 0: #!!!!!!!!
                            vehicle.path = self.RTV_system.PlanPath.PlanPath_CompleteSearch(vehicle, Trip())
                        # TODO if req cancelled on the vehicle, the req can be appended to the unmatched_requests or just give up
