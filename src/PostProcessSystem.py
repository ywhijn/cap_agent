...
class PostProcessSystem:
    ...
    def DrawVehicles(self, ax, vehicles, v_size, colors = ['darkolivegreen', 'darkviolet']):
        ...
        return ax
    def Draw_Vehicle(self, ax, vehicle, v_size, colors = ['darkolivegreen', 'darkviolet']):
    ...
    # function: Draw the snapshot of the chosen vehicle and requests
    def DrawSoloSnapshot(self,ax, chosen_v, v_size = 0.004, s = 100, colors = ['lightslategray', 'darkviolet', 'red', 'orange', 'lime'], draw_route = True, draw_road_netwrod = True, speed_lim = [0, 20], axis_lim = None):
        ...
        if chosen_v:
            ...
            for i, request in enumerate(vehicle.current_requests):
                cir = plt.Circle(self.GetCircleCenter(xv, yv, v_size, i), radius=v_size / 4, color=c_req_del, fill=True,
                                 alpha=1)
                ax.add_patch(cir)
        ...
        plt.scatter(1000, 1000, s=s, c=c_req_w, marker='*', alpha=1,
                    label='Passengers waiting to be assigned (pooling)')
