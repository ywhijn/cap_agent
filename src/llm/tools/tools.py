...
class POIbyLocation(BaseTool):
    ...
    description = "Get the POIs and semantic description of a location based on latitude and longitude."
    args_schema: Type[BaseModel] = Position
    env = Any
    cfg = Any
...
#             request_id, driver_id = args
#         elif 'request_id' in kwargs and 'driver_id' in kwargs:
#             request_id = kwargs['request_id']
#             driver_id = kwargs['driver_id']
#         else:
#             return "Invalid input. Please provide both request_id and driver_id."
#         try:
#             request_id = int(request_id)
#             driver_id = int(driver_id)
#         except ValueError:
...
#         return distance_time
    ...
    # async def _arun(self, request_id: int, driver_id: int) -> str:
    #     # 如果需要异步支持，可以在这里实现异步版本
    #     raise NotImplementedError("GetDistanceTimeByID does not support async")
# class GetDistanceTimeByID(BaseTool):
#     name ="get_distance_time_by_id" # "Get Travel Distance and Time By ID" - wrong name tool identifier error, should be a valid function name identifier,
#     description = "Get the distance and travel time between a request's position and a driver's position using their IDs"
#     args_schema: Type[BaseModel] = UV_ID_pair
#     env: Any
#     request_dict: Dict[int, list] = {78942: [104.0412682, 30.6669185], 81088: [104.0562247, 30.6223237]}
...
#         super().__init__(**kwargs)
#         self.env = env
#     # def test_tool(self, request_id: int, driver_id: int) -> Tuple[str, List[float]]:
#     #     if request_id not in self.request_dict or driver_id not in self.driver_dict:
#     #         print("The request ID or driver ID is not valid.")
#     #         request_id = 78942
#     #         driver_id = 17
...
#     #     round_digit = 2
#     #     distance_time = f"The distance and travel time between two points: Distance: {round(dis, round_digit)}, Time: {round(time, round_digit)}"
#     #     return distance_time, (dis, time)
#     @tool
#     def _run(self, *args: Any, **kwargs: Any) -> str:# Tuple[str, List[float]]:
#         """
#         Get the distance and travel time between a request's position and a driver's position.
#
#         Args:
...
#             str: A string describing the distance and travel time
#         """
#         # request_dict = {78942: [104.0412682, 30.6669185], 81088: [104.0562247, 30.6223237]} request_id
